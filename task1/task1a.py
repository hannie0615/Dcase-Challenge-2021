...
def main(argv):
    ...
    args, overwrite = handle_application_arguments(
        ...
        application_title='Task 1A: low-complexity ASC with Multiple Devices',
        version=__version__
    )
    ...
    if args.dataset_path:
        ...
        # Make sure given path exists (경로 없으면 만들기)
    ...
    dcase_util.utils.setup_logging(
        logging_file=os.path.join(param.get_path('path.log'), 'task1a.log') #log
    )
    ...
    # Log title
    log.title('DCASE2021 / Task1A -- low-complexity ASC with Multiple Devices')
    log.line()
        ...
        for item_id, audio_filename in enumerate(db.audio_files):
            ...
            if not os.path.isfile(feature_filename) or overwrite:
                ...
                audio = dcase_util.containers.AudioContainer().load(
                    ...
                    mono=True,
                    fs=param.get_path('feature_extractor.fs')   # 44100
                )
...
def do_learning(db, folds, param, log, overwrite=False):
    ...
    for fold in folds:
        ...
        fold_model_filename = os.path.join(
            ...
            'model_quant_fold_{fold}.h5'.format(fold=fold)
        )
    ...
    for fold in folds:
        ...
        if not os.path.isfile(fold_results_filename) or overwrite:
            ...
            if not keras_model:
                ...
                keras_model = tf.keras.models.load_model(fold_model_filename)
            ...
            for item in db.test(fold=fold):
                ...
                if len(keras_model.input_shape) == 4:
                    ...
                    if isinstance(keras_model.get_config(), list):
                        ...
                        print('data_format1'+str(data_format))
    ...
    for fold in folds:
        ...
        if embedding_size:
            log.row(
                ...
                (1 - (embedding_size['parameters']['non_zero']['count'] / float(
                    embedding_size['parameters']['all']['count']))) * 100.0,
        ...
        log.row('Acoustic model',
                ...
                (1 - (model_size['parameters']['non_zero']['count'] / float(
                    model_size['parameters']['all']['count']))) * 100.0,
